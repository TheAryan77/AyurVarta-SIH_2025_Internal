...
// props: title, options: [{label, value, image}], selected, onSelect
export default function TriCardQuestion({ title, options, selected, onSelect, circular }) {
  return (
  ...
  <div className={quizStyles.bodyTypeOptions}>
    ...
    className={`${quizStyles.bodyTypeCard} ${selected === opt.value ? quizStyles.selected : ''}`}
          ...
          >
  <img src={opt.image} alt={opt.label} className={circular ? styles.circleImg : undefined} />
